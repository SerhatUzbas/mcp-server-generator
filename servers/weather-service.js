...
if (!OPENWEATHER_API_KEY) {
  console.error("Error: OPENWEATHER_API_KEY is required. Please add it to your .env file.");
  process.exit(1);
}
...
server.tool(
  "getCurrentWeather",
  {
    ...
    units: z.enum(["metric", "imperial"]).default("metric").describe("Temperature units (metric: Celsius, imperial: Fahrenheit)")
  },
  async ({ city, units }) => {
    try {
      ...
      if (!response.ok) {
        ...
        return {
          content: [{ 
            type: "text", 
            text: `Error: ${errorData.message || "Failed to fetch weather data"}` 
          }],
          isError: true
        };
      }
...
server.tool(
  "getForecast",
  {
    city: z.string().min(1).describe("The name of the city to get forecast for"),
    units: z.enum(["metric", "imperial"]).default("metric").describe("Temperature units (metric: Celsius, imperial: Fahrenheit)"),
    days: z.number().int().min(1).max(5).default(3).describe("Number of days for the forecast (1-5)")
  },
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          content: [{ 
            type: "text", 
            text: `Error: ${errorData.message || "Failed to fetch forecast data"}` 
          }],
          isError: true
        };
      }
...
function formatForecastData(data, units, days) {
  ...
  data.list.forEach(forecast => {
    ...
    if (!forecastsByDay[day] || Math.abs(hour - 12) < Math.abs(new Date(forecastsByDay[day].dt * 1000).getHours() - 12)) {
