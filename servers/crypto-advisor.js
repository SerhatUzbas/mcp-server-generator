...
const server = new McpServer({
  ...
  version: "1.0.0",
  description:
    "Real-time cryptocurrency data and personalized investment insights",
});
...
const getCmcApiKey = () => {
  ...
  if (!apiKey) {
    throw new Error(
      "COINMARKETCAP_API_KEY environment variable is not set. Please configure it in Claude Desktop."
    );
  }
  return apiKey;
};
...
server.tool(
  ...
  {
    strategy: z
      .enum(["buy", "sell", "short", "long"])
      .describe("Investment strategy (buy, sell, short, long)"),
    timeInterval: z
      .enum(["short-term", "medium-term", "long-term"])
      .describe("Time horizon for the investment"),
    limit: z
      .number()
      .min(1)
      .max(100)
  ...
  async ({ strategy, timeInterval, limit, riskTolerance }) => {
    try {
      ...
      const marketMetrics = await fetchFromCMC(
...
server.tool(
  "getCryptoDetails",
  {
    ...
    convert: z
      ...
      .describe("Currency to convert prices to"),
  },
    ...
    try {
      ...
      if (!response || !response.data || !response.data.data) {
        return {
          content: [
            {
              type: "text",
              text: `Error fetching data for ${symbolFormatted}: Invalid API response structure`,
            },
          ],
          isError: true,
        };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error fetching data for ${symbol}: ${
  ...
  let scoredCoins = coins.map((coin) => {
    ...
    for (const window of relevantTimeWindows) {
      if (quote[window]) {
        ...
        if (strategy === "buy" || strategy === "long") {
          ...
          if (quote[window] > 10) {
            reasons.push(
              `Strong upward momentum: ${window.replace(
                "percent_change_",
                ""
              )} change of ${quote[window].toFixed(2)}%`
            );
          }
        }
    ...
    if (riskTolerance === "low") {
      ...
      if (coin.cmc_rank <= 20) {
        score += 20;
        reasons.push(
          "Large market cap (top 20) provides reduced volatility risk"
        );
      }
    ...
    if (
      (marketCapChange24h > 0 && (strategy === "buy" || strategy === "long")) ||
      (marketCapChange24h < 0 && (strategy === "sell" || strategy === "short"))
    ) {
      score += 10;
      reasons.push(
        `Aligned with overall market trend (${marketCapChange24h.toFixed(
          2
        )}% 24h change)`
      );
    }
...
server.tool(
  "getMarketOverview",
  {
    convert: z
      .string()
      .default(DEFAULT_CURRENCY)
      .describe("Currency for market data"),
  },
  async ({ convert }) => {
    try {
      ...
      const overview = {
        ...
        market_data: {
          ...
          market_cap_change_24h:
            marketData.total_market_cap_yesterday_percentage_change,
          volume_change_24h:
            marketData.total_volume_24h_yesterday_percentage_change,
        },
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error fetching market overview: ${error.message}`,
          },
        ],
        isError: true,
      };
...
function determineMarketSentiment(marketData) {
  ...
  if (capChange > 5) {
    ...
    explanation = `Market cap increased significantly by ${capChange.toFixed(
      2
    )}% in the last 24 hours.`;
  } else if (capChange > 2) {
    sentiment = "Bullish";
    explanation = `Market cap increased by ${capChange.toFixed(
      2
    )}% in the last 24 hours.`;
  } else if (capChange > -2) {
    sentiment = "Neutral";
    explanation = `Market cap relatively stable with ${capChange.toFixed(
      2
    )}% change in the last 24 hours.`;
  } else if (capChange > -5) {
    sentiment = "Bearish";
    explanation = `Market cap decreased by ${Math.abs(capChange).toFixed(
      2
    )}% in the last 24 hours.`;
  } else {
    sentiment = "Strongly Bearish";
    explanation = `Market cap decreased significantly by ${Math.abs(
      capChange
  ...
  if (volumeChange > 20 && capChange > 0) {
    explanation += ` Trading volume increased by ${volumeChange.toFixed(
      2
    ...
    explanation += ` Trading volume decreased by ${Math.abs(
      volumeChange
    ).toFixed(2)}%, indicating reduced market activity.`;
  }
...
server.tool(
  "analyzeTrends",
  {
    symbols: z
      .string()
      .describe(
        "Comma-separated list of cryptocurrency symbols (e.g., BTC,ETH,XRP)"
      ),
    convert: z
      .string()
      .default(DEFAULT_CURRENCY)
    ...
    timeInterval: z
      .enum(["short-term", "medium-term", "long-term"])
      .default("medium-term")
      .describe("Time interval for analysis"),
  },
  async ({ symbols, convert, timeInterval }) => {
    try {
      ...
      if (symbolArray.length === 0 || symbolArray.length > 10) {
        throw new Error(
          "Please provide between 1 and 10 cryptocurrency symbols"
        );
      }
      ...
      for (const symbol of symbolArray) {
        ...
        const marketChange = {
          "short-term":
            marketData.data.quote[convert]
              .total_market_cap_yesterday_percentage_change,
          "medium-term":
            marketData.data.quote[convert]
              .total_market_cap_7d_percentage_change,
          "long-term":
            marketData.data.quote[convert]
              .total_market_cap_30d_percentage_change,
        };
        ...
        analyses.push({
          ...
          trend_summary: `${
            coin.name
          } is in a ${trendDirection.toLowerCase()} with ${volatility.toLowerCase()} volatility, ${
            outperformingMarket ? "outperforming" : "underperforming"
          } the overall market over this ${timeInterval}.`,
        });
      }
      ...
      const result = {
        ...
        market_trend: {
          ...
          trend:
            relevantMarketChange > 5
              ? "Strong Uptrend"
              : relevantMarketChange > 1
              ? "Moderate Uptrend"
              : relevantMarketChange > -1
              ? "Sideways/Neutral"
              : relevantMarketChange > -5
          ...
          topPerformer && worstPerformer
            ? {
                top_performer: `${topPerformer.name} (${
                  topPerformer.symbol
                }) with ${topPerformer.analysis.average_change.toFixed(
                  2
                )}% average change`,
                worst_performer: `${worstPerformer.name} (${
                  worstPerformer.symbol
                }) with ${worstPerformer.analysis.average_change.toFixed(
                  2
                )}% average change`,
                relative_strength: `The difference between top and bottom performers is ${(
                  topPerformer.analysis.average_change -
                  worstPerformer.analysis.average_change
                ).toFixed(2)}%`,
              }
            : null,
      ...
      return {
        content: [
          { type: "text", text: `Error analyzing trends: ${error.message}` },
        ],
        isError: true,
      };
    }
