...
server.resource(
  ...
  async (uri, {id}) => {
    ...
    const products: Record<string, { name: string; price: number; description: string }> = {
      "1": { name: "Laptop", price: 999, description: "Powerful laptop with 16GB RAM" },
      "2": { name: "Smartphone", price: 699, description: "Latest smartphone with 5G" },
      "3": { name: "Tablet", price: 499, description: "10-inch tablet with long battery life" }
    };
    
    const product = products[id as string] || { name: "Unknown", price: 0, description: "Product not found" };
